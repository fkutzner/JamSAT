...
then
  ...
  then
    cmake -DJAMSAT_LINK_VERBOSE=ON -DCMAKE_PREFIX_PATH=${PWD}/../minisat-install -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=Debug -DJAMSAT_ENABLE_COVERAGE=ON -DJAMSAT_DISABLE_OPTIMIZATIONS=ON ${TRAVIS_BUILD_DIR}
  ...
  then
    ...
    cmake -DJAMSAT_LINK_VERBOSE=ON -DCMAKE_PREFIX_PATH=${PWD}/../minisat-install -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=Debug -DJAMSAT_ENABLE_SANITIZERS=ON -DJAMSAT_DISABLE_OPTIMIZATIONS=ON -DJAMSAT_USE_SEPARATE_CLANG_SANITIZERS=ON ${TRAVIS_BUILD_DIR}
    build_and_test
    ...
    cmake -DJAMSAT_LINK_VERBOSE=ON -DCMAKE_PREFIX_PATH=${PWD}/../minisat-install -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=Debug ${TRAVIS_BUILD_DIR}
    build_and_test
  fi
else
  cd ${TRAVIS_BUILD_DIR}
  cmake -DJAMSAT_LINK_VERBOSE=ON -DCMAKE_PREFIX_PATH=${PWD}/../minisat-install -DCMAKE_BUILD_TYPE=Debug -DJAMSAT_ENABLE_COVERAGE=ON -DJAMSAT_DISABLE_OPTIMIZATIONS=ON .
